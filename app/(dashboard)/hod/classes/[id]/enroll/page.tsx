...
import { ChevronLeft, Search, UserPlus, Users, AlertTriangle, Loader2 } from "lucide-react";
...
export default function HodEnrollStudentsPage() {
  ...
  const fetchAvailableStudents = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("auth_token");
      const response = await fetch("/api/students?includeEnrollments=true&mode=all", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((student) => {
      ...
      // Enrollment filter
      const hasEnrollment = student.currentEnrollments && student.currentEnrollments.length > 0;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-muted-foreground">
            {classData ? `${classData.grade.name} ${classData.name}` : "Select students to enroll"}
          </p>
        </div>
      </div>
          ...
          {filteredStudents.length === 0 ? (
            ...
            <div className="space-y-2 max-h-[500px] overflow-y-auto">
              {filteredStudents.map((student) => {
                ...
                const hasOtherEnrollment = student.currentEnrollments && student.currentEnrollments.length > 0;
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {hasOtherEnrollment && (
                          <span className="text-xs text-amber-600">
                            • Enrolled in {student.currentEnrollments![0].class.grade.name} {student.currentEnrollments![0].class.name}
                          </span>
                        )}
                      </div>
                    </div>

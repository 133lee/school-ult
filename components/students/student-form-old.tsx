...
// Form validation schema aligned with backend validation
// Student number format: STU-YYYY-NNNN (auto-generated and validated server-side)
...
export function StudentForm({
  ...
  // Date input states - moved outside FormField.render to prevent re-creation
  ...
  // Generate student number on mount - matches backend format: STU-YYYY-NNNN
  ...
  // Sync date input states when form values change
  useEffect(() => {
    ...
    const admission = form.getValues("admissionDate");
    if (admission) {
      setAdmissionInput(format(admission, "dd/MM/yyyy"));
      setAdmissionMonth(admission);
    }
  }, [form]);
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col md:col-span-2">
                      ...
                      <div className="flex gap-2">
                        <FormControl>
                          <Input
                            ...
                            onBlur={() => {
                              // Reset to formatted date on blur if valid
                              if (field.value) {
                                setDobInput(format(field.value, "dd/MM/yyyy"));
                              }
                            }}
                            className="flex-1"
                          />
                      ...
                      <FormDescription>
                        Type DD/MM/YYYY or use calendar. Student must be between 3 and 25 years old
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                        ...
                        <FormControl>
                          <Input
                            ...
                            onBlur={() => {
                              // Reset to formatted date on blur if valid
                              if (field.value) {
                                setAdmissionInput(format(field.value, "dd/MM/yyyy"));
                              }
                            }}
                            className="flex-1"
                          />
                        </FormControl>
                        <Popover open={admissionPopoverOpen} onOpenChange={setAdmissionPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button
                              variant="outline"
                              type="button"
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <Calendar
                              ...
                              onMonthChange={setAdmissionMonth}
                              onSelect={(date) => {
                                if (!date) return;
                                field.onChange(date);
                                setAdmissionInput(format(date, "dd/MM/yyyy"));
                                setAdmissionMonth(date);
                                setAdmissionPopoverOpen(false);
                              }}
                              captionLayout="dropdown"
                              fromYear={2000}
                      ...
                      <FormDescription>
                        Type DD/MM/YYYY or use calendar
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                    ...
                    <div>
                      ...
                      <dd className="font-semibold mt-1">
                        {form.getValues("admissionDate")
                          ? format(form.getValues("admissionDate"), "PPP")
                          : "N/A"}
                      </dd>
                    </div>
                    <div>
